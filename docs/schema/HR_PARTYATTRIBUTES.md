...
Party Attributes - Contains required details about the properties of the party.  
...
| ---- | ---- | ---------- |
| PK_PARTYATTRIBUTES | PRIMARY KEY | CLUSTERED, unique, part of a PRIMARY KEY constraint, [ ID ] |
| FK_PARTYATTRIBUTES_PARTYTYPE | FOREIGN KEY | FOREIGN KEY(PARTY_ID) REFERENCES HR_PARTY(ID) ON UPDATE NO_ACTION ON DELETE NO_ACTION |

## Indexes

| Name | Definition |
| ---- | ---------- |
| PK_PARTYATTRIBUTES | CLUSTERED, unique, part of a PRIMARY KEY constraint, [ ID ] |
| IDX_PARTYATTRIBUTES | NONCLUSTERED, unique, [ PARTY_ID, EFFECTIVEFROM ] |

## Relations

![er](HR_PARTYATTRIBUTES.svg)
